...
export class ChallengesService {
  private _challengeDeployStateChanged$ = new Subject<{ id: string, isDeployed: boolean }>();
  ...
  public deploy(challenge: { id: string }): Observable<Challenge> {
    return this.http.put<Challenge>(this.apiUrl.build("challenge/start"), challenge).pipe(tap(challenge => {
      this._challengeDeployStateChanged$.next({ id: challenge.id, isDeployed: challenge.hasDeployedGamespace });
    }));
  }
